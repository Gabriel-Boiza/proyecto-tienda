document.addEventListener('DOMContentLoaded', function(event){
    ...
    function filterProducts() {
        // Consider adding aria-label="Buscar productos" to the input in your HTML
        const searchTerm = document.getElementById('searchInput').value.toLowerCase();
        // Consider adding aria-label="Filtrar por categoría" to the select in your HTML
    ...
    function updateTable(products) {
        ...
        products.forEach(producto => {
            ...
            verBtn.setAttribute('aria-label', `Ver detalles del producto ${producto.nombre}`);
            verBtn.innerHTML = `
            ...
            // Add aria-label for accessibility
            editarBtn.setAttribute('aria-label', `Editar producto ${producto.nombre}`);
            editarBtn.innerHTML = `
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5" aria-hidden="true">
            ...
            // Add aria-label for accessibility
            eliminarBtn.setAttribute('aria-label', `Eliminar producto ${producto.nombre}`);
    ...
    fetch('/api/productos')
        ...
        .then(data => {
            ...
            // Clear existing options before adding new ones (optional but good practice)
            categoryFilter.innerHTML = '<option value="">Todas las categorías</option>'; // Add default option
            categories.forEach(category => {
                const option = document.createElement('option');
                option.value = category.toLowerCase(); // Use lowercase value for consistency
    ...
    // Remember to add aria-label="Buscar productos" to the input in your HTML
    document.getElementById('searchInput').addEventListener('input', filterProducts);
    // Remember to add aria-label="Filtrar por categoría" to the select in your HTML
    ...
    function eliminarProducto(id) {
        ...
        const productName = productToDelete ? productToDelete.nombre : 'este producto';

        if (confirm(`¿Estás seguro de que deseas eliminar ${productName}?`)) {
            fetch(`/api/productos/${id}`, { // Ensure the URL is correct, likely needs /api prefix
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'), // Use getAttribute
                    'Accept': 'application/json' // Often needed for API responses
                }
            })
            .then(response => {
            ...
            .then(result => {
                if (result.ok) {
                    console.log('Producto eliminado:', result.body);
                    ...
                    console.error('Error al eliminar el producto:', result.status, result.body);
